...
Approximate number of tax_id-s belonging to virus or bacteriophage class. Putative bacterial virus names were detected using following regular expression: "phag|bac|esch|coccu|chla|mona|erw|lis|vibr|shi|yers|salm|phi|clos|chla".
```{r}
phage_or_virus <- taxa_sci %>%
  mutate(name_txt = str_to_lower(name_txt),
    Class = case_when(
    (str_detect(name_txt, "vir") & str_detect(name_txt, "phag|bac|esch|coccu|chla|mona|erw|lis|vibr|shi|yers|salm|phi|clos|chla")) ~ "phage",
